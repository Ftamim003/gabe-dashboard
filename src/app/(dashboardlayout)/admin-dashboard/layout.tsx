...
const sidebarItems = [
  ...
  {
    icon: Users,
    label: "Users And Account",
    href: "/admin-dashboard/userInfo",
  },
  {
    icon: Users,
    label: "Squad Members",
    href: "/admin-dashboard/squad-member",
  },
  {
    icon: CreditCard,
  ...
  // { icon: Video, label: "Upload Video", href: "/admin-dashboard/upload-video" },
  {
    icon: Settings,
    label: "Settings",
    href: "/admin-dashboard/admin-information",
  },
];
...
export default function AdminDashboardLayout({
  ...
  const { data: notification } = useGetNotificationsQuery("un", {
    pollingInterval: 100,
  });
  const unreadCount =
    notification?.data?.filter((n: { read: boolean }) => !n.read).length || 0;
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 text-gray-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-gray-300">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="lg:hidden text-gray-800 hover:bg-gray-300"
        ...
        {/* Main nav with logout at bottom */}
        <div className="flex flex-col h-[calc(100%-64px)] justify-between">
          ...
          <div className="p-4 border-t border-gray-300">
            <Button
              variant="ghost"
              className="w-full justify-start text-gray-700 hover:bg-red-100 hover:text-red-600"
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-7">
              ...
              <Avatar>
                <AvatarImage
                  src={
                    adminInfo?.data.profilePic ||
                    "/placeholder.svg?height=40&width=40"
                  }
                />
                <AvatarFallback>
                  {adminInfo?.data.fullName?.[0] || "A"}
                </AvatarFallback>
              </Avatar>
            </div>
          </div>
